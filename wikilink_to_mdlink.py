...
def process_wikilink(match, file_path, file_map):
    ...
    if target_file_path is None:
        print(
            f"Warning: File {target_filename} not found for WikiLink '{wikilink}' in file {file_path}."
        )
        return f"[{title}]({filename.replace(' ', '%20')}{target_extension})"

    rel_path = os.path.relpath(
        target_file_path, start=os.path.dirname(file_path)
    ).replace(" ", "%20")
    print(
        f"Processing WikiLink '{wikilink}' in file {file_path}: [{title}]({rel_path})"
    )
...
def convert_wikilinks(file_path, file_map):
    ...
    content = re.sub(
        r"\[\[(.*?)\]\]",
        lambda match: process_wikilink(match, file_path, file_map),
        content,
    )
