    ...
    for root, _, files in os.walk(directory_path):
        for file in files:
            if file.endswith(".md"):
                ...
                modified_content, is_modified = remove_related_heading_and_content(
                ...
                if is_modified:
                    ...
                    confirm = input(
                        "Do you want to remove the heading and its content? (Y/N): "
                    ).lower()
                    if confirm == "y":
                        with open(file_path, "w", encoding="utf-8") as file:
                            file.write(modified_content)
                        print(
                            f"Successfully removed the heading and its content from {file_path}"
                        )
                    else:
